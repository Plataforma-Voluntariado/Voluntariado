...
function HomeVolunteerLayout(){
    ...
    const mapApiRef = useRef(null); // ref para controlar el mapa
    ...
    return(
        <div className="home-volunteer-layout">
            ...
            {volunteerings && volunteerings.length > 0 ? volunteerings.map((volunteering) => (
                <VolunteeringCard 
                    key={volunteering.id_voluntariado}
                    volunteering={volunteering}
                    onFocusMap={() => handleFocusOnMap(volunteering.ubicacion, 15)}
                />
            )) : !loading ? <p>No se encontraron eventos.</p> : null}
        </div>
    )
}
