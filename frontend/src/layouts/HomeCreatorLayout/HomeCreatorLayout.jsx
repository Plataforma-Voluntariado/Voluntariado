...
function HomeCreatorLayout() {
    ...
    const getStatistics = () => {
        ...
        const activos = voluntariados.filter(v => v.estado === "ACTIVO").length;
        const totalInscripciones = voluntariados.reduce((sum, v) => sum + (v.inscripciones?.length || 0), 0);
        
        return { total, activos, totalInscripciones };
    };
            ...
            <div className="creator-content">
                <div className="content-header">
                    ...
                    {voluntariados.length > 0 && (
                        <p>{voluntariados.length} voluntariado{voluntariados.length !== 1 ? 's' : ''} encontrado{voluntariados.length !== 1 ? 's' : ''}</p>
                    )}
                </div>
                ...
                {!error && voluntariados.length === 0 && (
                    <div className="empty-state">
                        <div className="empty-icon">
                            <FaCalendarAlt />
                        </div>
                        <h3>¡Comienza tu primer voluntariado!</h3>
                        <p>Aún no has creado ningún voluntariado. Crea uno ahora y comienza a conectar con voluntarios.</p>
