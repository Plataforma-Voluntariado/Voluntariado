...
export const createVoluntariado = async (voluntariadoData, fotos = []) => {
    try {
        ...
        if (error.response?.status === 401) throw "No tienes permisos para crear voluntariados. Verifica que tengas el rol de CREADOR.";
        if (error.response?.status === 403) throw "Acceso denegado. Solo los usuarios CREADOR pueden crear voluntariados.";
        throw handleError(error, "Error al crear voluntariado");
    }
};
...
export const getMyVoluntariados = async () => {
    try {
        ...
        throw handleError(error, "Error al obtener voluntariados");
    }
};
...
export const getVoluntariadoById = async (id) => {
    try {
        ...
        throw handleError(error, "Error al obtener voluntariado");
    }
};
