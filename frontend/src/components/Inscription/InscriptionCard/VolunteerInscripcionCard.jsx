...
const VolunteerInscripcionCard = ({
  ...
  const puedeCancelar =
    ESTADO_INSCRIPCION === "PENDIENTE" && ESTADO_VOLUNTARIADO === "PENDIENTE";
  const puedeComentar =
    inscripcion.asistencia === true && !inscripcion.calificado;
  ...
  const handleCancelar = async () => {
    try {
      ...
      await WrongAlert({
        message: error.message || "No se pudo cancelar la inscripción.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleEnviarComentario = async (comentario) => {
    try {
      await crearResenaVoluntariado(inscripcion.voluntariado.id_voluntariado, {
        comentario,
      });
      setModalOpen(false);
      refreshInscripciones();
      ...
      await WrongAlert({
        message: error.message || "No se pudo enviar el comentario.",
      });
    }
  };
  ...
  return (
    ...
    >
      ...
      <img
        src={voluntariado?.creador.url_imagen}
      ...
      <div className="inscripcion-card-info">
        ...
        <p>
          <MdDateRange className="inscripcion-icon" />
          {new Date(inscripcion.fecha_inscripcion).toLocaleDateString()}
        </p>
        <p>
          <MdEmail className="inscripcion-icon" />
          {voluntariado?.creador.correo}
        </p>
      </div>

      <div className="inscripcion-card-indicators">
        {inscripcion.asistencia && (
          <span className="indicator asistencia">
            <FaCheckCircle /> Asistió
          </span>
        )}
        {inscripcion.calificado && (
          <span className="indicator calificado">
            <FaStar /> Calificado
          </span>
        )}
      </div>
      ...
      <div className="inscripcion-card-actions">
        ...
        {puedeComentar && (
          <button onClick={() => setModalOpen(true)} className="comentar-btn">
